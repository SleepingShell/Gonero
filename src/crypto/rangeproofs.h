...
typedef unsigned int bits[64];
typedef ec_scalar key;          //same as ec_scalar
typedef key key64[64];          //64 keys (represent [0...2^64) )

typedef struct borromean_sig {
    key64 s0;
    key64 s1;
    key e0;
} borromean_sig;

typedef struct range_proof {
    key64 Ci;
    borromean_sig sig;
} range_proof;
